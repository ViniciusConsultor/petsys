...
[assembly: WebResource("BoletoNet.Imagens.logo-boleto.jpg", "image/jpg")]
...
namespace BoletoNet
{
    ...
    internal class Banco_Caixa : AbstractBanco, IBanco
    {
        ...
        public override void FormataCodigoBarra(Boleto boleto)
        {
            ...
            if (boleto.Carteira.Equals("SR") || boleto.Carteira.Equals("CR"))
            {
                ...
                if (boleto.NossoNumero.Length == 10)
                {
                    ...
                    var digitoConta = boleto.Cedente.ContaBancaria.DigitoConta;
                    var carteira = "SR";

                    var nossoNumero = boleto.NossoNumero;

                    var digitoVerificadorNossoNumero = Mod11Base9(nossoNumero).ToString();

                    var nosoNumeroMaisDigitoVerificador = nossoNumero + digitoVerificadorNossoNumero;

                    var codigoCedente = boleto.Cedente.Codigo;

                    var digitoVerificadorCodigoCedente = boleto.Cedente.DigitoCedente;

                    var agenciaContaCedente = agencia + codigoCedente;

                    // 43 numeros para o calculo do digito verificador do codigo de barras
                    var digitoVerificadorCodigoBarras = banco + moeda + fatorVencimento.ToString() + valorDocumento +
                                                        nossoNumero + agenciaContaCedente;

                    var digGeral = Mod11(digitoVerificadorCodigoBarras, 9).ToString();

                    _dacBoleto = digGeral;

                    campoLivre = nossoNumero + agenciaContaCedente;
        ...
        public override void FormataLinhaDigitavel(Boleto boleto)
        {
            ...
            //    var banco = boleto.CodigoBarra.Codigo.Substring(0, 3);
            //    var moeda = boleto.CodigoBarra.Codigo.Substring(3, 1);
            //    var nossoNumero = boleto.NossoNumero;
            //    var agenciaCedente = Utils.FormatCode(boleto.Cedente.ContaBancaria.Agencia, 4);
            //    var codigoLivre2Dig = "";
            //    var codigoCedente = boleto.Cedente.Codigo.Substring(0, 10);
        ...
        public override void ValidaBoleto(Boleto boleto)
        {
            ...
            if(boleto.Carteira.Equals("SR") && boleto.NossoNumero.Length == 10)
            {
                boleto.Cedente.DigitoCedente = Convert.ToInt32(boleto.Cedente.ContaBancaria.DigitoConta);
            }
            else if (boleto.Cedente.DigitoCedente == -1)
