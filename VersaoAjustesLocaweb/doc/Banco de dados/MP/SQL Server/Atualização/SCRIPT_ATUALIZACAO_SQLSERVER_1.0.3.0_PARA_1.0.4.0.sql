...
ALTER TABLE NCL_CEDENTE ADD IMAGEMBOLETO VARCHAR(255) NULL;

DROP TABLE FN_CNFBOLETO;

DELETE FROM NCL_MENUFUNCAO 
WHERE IDMODULO = 'MOD.FN' AND IDFUNCAO = 'FUN.FN.003' 
AND IMAGEM = 'bogus' AND URL = 'FN/frmConfiguracaoDoBoleto.aspx' AND AGRUPADOR = 'Configurações';

DELETE FROM NCL_OPERACAO 
WHERE IDMODULO = 'MOD.FN' AND IDFUNCAO = 'FUN.FN.003' 
AND IDOPERACAO = 'OPE.FN.003.0001' AND NOME = 'Salvar';

DELETE FROM NCL_FUNCAO 
WHERE IDMODULO = 'MOD.FN' AND IDFUNCAO = 'FUN.FN.003' AND NOME = 'Configurações do Boleto';