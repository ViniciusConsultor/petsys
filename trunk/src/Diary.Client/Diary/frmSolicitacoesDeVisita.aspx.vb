...
Partial Public Class frmSolicitacoesDeVisita
    ...
    Private Sub CarregaSolicitacoesSemFiltro()
        ...
        Using Servico As IServicoDeSolicitacaoDeVisita = FabricaGenerica.GetInstancia.CrieObjeto(Of IServicoDeSolicitacaoDeVisita)()
            Solicitacoes = Servico.ObtenhaSolicitacoesDeVisita(chkConsiderarSolicitacoesFinalizadas.Checked)
        End Using
    ...
    Private Sub ExibaSolicitacoes(ByVal Solicitacoes As IList(Of ISolicitacaoDeVisita))
    ...
    Protected Sub btnNovo_Click()
        ...
        URL = ObtenhaURL()
        ScriptManager.RegisterStartupScript(Me, Me.GetType(), New Guid().ToString, UtilidadesWeb.ExibeJanelaModal(URL, "Nova solicitação de visita"), False)
    End Sub
    ...
    Private Sub grdItensLancados_ItemCommand(ByVal source As Object, ByVal e As Telerik.Web.UI.GridCommandEventArgs) Handles grdItensLancados.ItemCommand
        ...
        ElseIf e.CommandName = "Modificar" Then
            ...
            ScriptManager.RegisterStartupScript(Me, Me.GetType(), New Guid().ToString, UtilidadesWeb.ExibeJanelaModal(URL, "Cadastrar solicitação de visita"), False)
        ...
        ElseIf e.CommandName = "Despachar" Then
            ...
            ScriptManager.RegisterStartupScript(Me, Me.GetType(), New Guid().ToString, UtilidadesWeb.ExibeJanelaModal(URL, "Despachar solicitação de visita"), False)
        End If
    End Sub
    ...
    Private Sub Page_PreRender(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.PreRender
        ...
        grdItensLancados.Columns(0).Visible = Principal.EstaAutorizado("OPE.DRY.004.0002")

        'Coluna com botão excluir
        grdItensLancados.Columns(1).Visible = Principal.EstaAutorizado("OPE.DRY.004.0003")

        'Coluna com botão despachar
        grdItensLancados.Columns(8).Visible = Principal.EstaAutorizado("OPE.DRY.004.0004")

        'Coluna com botão finalizar
        grdItensLancados.Columns(9).Visible = Principal.EstaAutorizado("OPE.DRY.004.0005")
    ...
    Protected Sub btnPesquisar_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles btnPesquisar.Click
        If txtDataInicial.IsEmpty Then
            ScriptManager.RegisterClientScriptBlock(Me, Me.GetType(), New Guid().ToString, UtilidadesWeb.MostraMensagemDeInconsitencia("A data inicial da solicitação de visita deve ser informada."), False)
            Exit Sub
        End If

        If txtDataFinal.IsEmpty Then
            ScriptManager.RegisterClientScriptBlock(Me, Me.GetType(), New Guid().ToString, UtilidadesWeb.MostraMensagemDeInconsitencia("A data final da solicitação de visita deve ser informada."), False)
            Exit Sub
        End If
        ...
        Using Servico As IServicoDeSolicitacaoDeVisita = FabricaGenerica.GetInstancia.CrieObjeto(Of IServicoDeSolicitacaoDeVisita)()
            Solicitacoes = Servico.ObtenhaSolicitacoesDeVisita(chkConsiderarSolicitacoesFinalizadas.Checked, _
    ...
    Protected Sub btnPesquisarPorCodigo_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles btnPesquisarPorCodigo.Click
        If String.IsNullOrEmpty(txtCodigoDaSolicitacao.Text) Then
            ScriptManager.RegisterClientScriptBlock(Me, Me.GetType(), New Guid().ToString, UtilidadesWeb.MostraMensagemDeInconsitencia("O código da solicitação de visita deve ser informado."), False)
            Exit Sub
        End If
        ...
        Using Servico As IServicoDeSolicitacaoDeVisita = FabricaGenerica.GetInstancia.CrieObjeto(Of IServicoDeSolicitacaoDeVisita)()
            Dim Solicitacao As ISolicitacaoDeVisita
    ...
    Private Sub btnPesquisarPorContato_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles btnPesquisarPorContato.Click
        If ctrlContato1.ContatoSelecionado Is Nothing Then
            ScriptManager.RegisterClientScriptBlock(Me, Me.GetType(), New Guid().ToString, UtilidadesWeb.MostraMensagemDeInconsitencia("O contato da solicitação de visita deve ser informado."), False)
            Exit Sub
        End If
        ...
        Using Servico As IServicoDeSolicitacaoDeVisita = FabricaGenerica.GetInstancia.CrieObjeto(Of IServicoDeSolicitacaoDeVisita)()
            Solicitacoes = Servico.ObtenhaSolicitacoesDeVisita(chkConsiderarSolicitacoesFinalizadas.Checked, ctrlContato1.ContatoSelecionado.Pessoa.ID.Value)
        End Using
